...
class CreatePatientAccount extends StatefulWidget {
  const CreatePatientAccount({super.key});
...
class _CreatePatientAccountState extends State<CreatePatientAccount> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar(
        title: const ExtraSmallText(text: 'Create Patient Account', size: 20, color: Colors.white),
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              CustomTextFormField(controller: countryController, hintText: 'Enter your country'),
              const SizedBox(height: 18),

              CustomTextFormField(controller: stateController, hintText: 'Enter your state'),
              const SizedBox(height: 18),
                    ...
                    onPressed: patientController.isLoading.value
                        ...
                        : () async {
                            ...
                            if (success) {
                              ...
                              final createdPatient = patientController.currentPatient.value;

                              if (createdPatient != null && createdPatient.id.isNotEmpty) {
                                storage.write('patientId', createdPatient.id);
                                print("📌 Saved new patientId: ${createdPatient.id}");
                              }

                              // Navigate and refresh
                              Get.offAllNamed(RoutesHelper.patientHomeScreen);
                            }
                          },
